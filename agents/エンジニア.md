...
| デプロイ | Vercel (`vercel --prod`) | https://kairox.jp |
...
```
frontend/
├── middleware.ts               旧URL → kairox.jp 301リダイレクト
├── app/
│   ├── narita/
│   │   ├── page.tsx            メイン予約画面（4言語・全ステップ・ライトテーマ）
│   │   ├── layout.tsx          OGP・メタタグ
│   │   └── status/[id]/        予約追跡ページ
│   ├── driver/
...
│   │   └── company/            KAIROX Intelligence API
│   ├── DriverView.tsx          ドライバー管理UI（PIN・GPS・フライトボード）
│   ├── AdminView.tsx           管理者UI（GPS学習・テスト予約・ゾーン選択）
│   ├── TrackingView.tsx        追跡UI（PlayerProgressBar・NearbyPlayersPanel・ReviewForm）
│   ├── globals.css             ブランドカラー変数（--kairox-blue: #0052ff 等）
...
- [ ] `docs/releases/リリースノート_vX.X.X.md` を更新（大きな変更のみ）
...
**対策:** TR の 4言語ブロック全てに同じキーを追加する。`ライター.md` のテンプレートを使う。
...
| 確認項目 | URL |
|---------|-----|
| メイン画面 | https://kairox.jp/narita |
| ドライバー画面 | https://kairox.jp/driver |
| 管理者画面 | https://kairox.jp/admin |
| 予約追跡 | https://kairox.jp/narita/status/KRX-TEST |
...
| 2026-03-27 | kairox.jp 移行。PostgreSQL化。middleware.ts追加。TrackingView/AdminView/ReviewForm追加 |
| 2026-03-28 | /narita ライトテーマ（#0052ff）。ブランドカラー変数。テロップ黒黄色。ディレクトリ構成更新 |
